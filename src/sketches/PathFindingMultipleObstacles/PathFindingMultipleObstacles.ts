...
class PathFindingMultipleObstacles extends SketchBase {
  ...
  setup(p5: p5Base) {
    ...
    // Obstacle creation
    [
      [p5.width / 2, p5.height / 2, 100],
      [p5.width / 3, (p5.height * 2) / 3, 50],
      [(p5.width * 2) / 3, p5.height / 8, 70],
      [(p5.width * 7) / 8, (p5.height * 4) / 8, 30],
    ].forEach((obstacle) => {
      this.obstacles.push(
        new ObstacleModel(
...
export { PathFindingMultipleObstacles as default };
